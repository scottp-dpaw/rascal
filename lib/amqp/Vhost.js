...
function Vhost(vhostConfig, components) {
  ...
  this.init = function (next) {
    ...
    init(vhostConfig, { connectionIndex: self.connectionIndex, components }, (err, config, ctx) => {
      ...
      debug('vhost: %s was initialised with connection: %s', self.name, self.connection._rascal_id);
  ...
  this.disconnect = function (next) {
    ...
    if (!self.connection) return next();
    ...
    const factory = {
      ...
      validate(channel) {
        return new Promise((resolve) => {
          resolve(!channel._rascal_closed && self.connection && self.connection.connection === channel.connection);
        });
      },
    };
  ...
  function createChannelWhenInitialised(confirm, next) {
    if (self.connection) return createChannel(confirm, next);
  ...
  function createChannel(confirm, next) {
    ...
    if (!self.connection) return next(new Error(format('Vhost: %s must be initialised before you can create a channel', self.name)));
    ...
    self.connection.once('close', closeHandler);
    self.connection.once('error', errorHandler);
    confirm ? self.connection.createConfirmChannel(callback) : self.connection.createChannel(callback);
    ...
    function callback(err, channel) {
      invocations++;
      self.connection && self.connection.removeListener('close', closeHandler);
      self.connection && self.connection.removeListener('error', errorHandler);
      if (err) {
      ...
      channel.connection._rascal_id = self.connection._rascal_id;
      channel.connection.setMaxListeners(0);
      debug('Created %s channel: %s from connection: %s', getChannelMode(confirm), channel._rascal_id, self.connection._rascal_id);
  ...
  function attachErrorHandlers(config) {
    ...
    const errorHandler = _.once(handleConnectionError.bind(null, self.connection, config));
    self.connection.on('error', errorHandler);
    self.connection.on('close', errorHandler);
  }
